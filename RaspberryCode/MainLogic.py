...
from KitRelease import MotoriRK
#from com_espNEW import LibESP
#from com_esp import LibESP
from EspCommunication import LibESP
import MainSensors as snv1
import RobotVariables as robot#contiene tutte le sottolibrerie dei movimenti 
from NeopixelLib import Neopix
import copy


import MazeClass as classemappa
        ...
        while(1):
            ...
            if(lg.avvio==0):
                ...
                if(lg.stato == 'primo90' and lg.Libero(0)):
                    ...
                    lg.stato = 'secondo90AHHA'
                    timeCoDoNo = time.time()
                elif(lg.stato == 'secondo90AHHA' and lg.Libero(0) and (time.time()-timeCoDoNo)>1 ):
                    lg.stato = 'secondo90'
                    controlloVittime.Avvia(0,1)
                    ...
                    if(casella.muri[mappa.limit04(mappa.sempG(robo[g]) + 3)] == 1 and casella.vittime[mappa.limit04(mappa.sempG(robo[g]) + 3)]==None ): #!='C'
                        print("ALLINEO")
                    ...
                    if(lg.sceltogira!=0 and abs(lg.sceltogira)!=180 and (lg.okCamSx==1 or lg.okCamDx==1)):
                        controlloVittime.Avvia(0,1)
                    ...
                    pidG.avvio = 1
                    dritto.Avvia(0,1.3,30,1) # normali 30
                    lg.indietroNero = 0
                    ...
                    if(lg.casella == lg.nero and lg.flNero==0):
                        ...
                        if(lg.distanzaColore>2):
                            lg.velSx = lg.velDx = 0 
                            lg.flNero = 1
                            dritto.Avvia(0,-1,lg.distanza-2)
                ...
                if(dritto.avvio): #se è stato attivato (non usare .avvio per disattivarlo)
                    if(dritto.stato==0): #se lo stato del dritto è lo 0
                        ...
                        if(dritto.modalita==1):
                            dritto.direzione = mappa.sempG(robo[g])
                            if(dritto.direzione == 0 or dritto.direzione==2):
                                dritto.coordinata = y
                                if(dritto.direzione == 0):
                                    dritto.correzione = 1
                                else:
                                    dritto.correzione = -1
                            else:
                                dritto.coordinata = x
                                if(dritto.direzione == 1):
                                    dritto.correzione = -1
                                else:
                                    dritto.correzione = 1
                            dritto.posizioneIniziale = robo[dritto.coordinata]
                        ...
                        if(dritto.modalita==1):
                            dritto.distanzaMod = (robo[dritto.coordinata]-dritto.posizioneIniziale) *30 * dritto.correzione
                        else:
                            dritto.distanzaMod = lg.distanza
                        if(dritto.velocita<0): #calcolo distaza percorsa relativamnte alla velocità impostata (avanti o indietro)
                            dritto.distanzaPercorsa = -dritto.distanzaMod
                        else:
                            dritto.distanzaPercorsa = dritto.distanzaMod
                ...
                if(controlloVittime.avvio):
                    ...
                    if(controlloVittime.stato==0):
                        ...
                        pidG.avvio = 0 
                        lg.avvioLaser = [1]*lg.nLaser
                        controlloVittime.stato=0.5
                        
                    if(controlloVittime.stato==0.5):
                        if((lg.laser[sn.lAvDx] < 15 or lg.laser[sn.lDiDx]<15 )
                            or lg.avvioCamDx[0]==0 or lg.okCamDx==0 ):
                            controlloVittime.stato=0.6
                        elif((time.time()-lg.tempTrovatoDx)<0.2):
                            if(lg.trovatoDx=="B"):
                                direz = mappa.limit04(mappa.sempG(robo[g])-1)
                                aggX = (direz%2)*(direz-2)  * -1
                                aggY = ((direz%2) -1) * (direz-1) * -1
                                casellaDopo = mappa.map.map[round(robo[x]-aggX)][round(robo[y]-aggY)]
                                casellaDopo.colore = lg.nero-1
                                casellaDopo.esplorata +=1
                                ...
                                controlloVittime.stato=0.6
                    if(controlloVittime.stato==0.6):
                        if((lg.laser[sn.lAvSx] < 15 or lg.laser[sn.lDiSx]<15 )
                            or lg.avvioCamSx[0]==0 or lg.okCamSx==0 ):
                            controlloVittime.stato=3
                        elif((time.time()-lg.tempTrovatoSx)<0.2 ):
                        ...
                        if((lg.laser[sn.lAvDx] > 15 or lg.laser[sn.lDiDx]>15 )
                            ...
                            or lg.avvioCamDx[0]==0 or lg.okCamDx==0 or
                            mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g]))] != None):
                            
                            controlloVittime.stato=2
                        elif( ((time.time()-lg.tempTrovatoDx)<0.2 and controlloVittime.modalita==0) or lg.fTrovatoDx ): #lg.fTrovatoDx
                            if(lg.trovatoDx!="N" and lg.trovatoDx!="B"):
                                pidG.avvio = 0 
                        ...
                        if((lg.laser[sn.lAvSx] > 15 or lg.laser[sn.lDiSx]>15 )
                            or mappa.map.map[round(robo[x])][round(robo[y])].colore != (lg.bianco -1)
                            or lg.avvioCamSx[0]==0 or lg.okCamSx==0 or
                            mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g])+2)]!= None):
                            
                            controlloVittime.stato=5
                        elif(((time.time()-lg.tempTrovatoSx)<0.2 and controlloVittime.modalita==0) or lg.fTrovatoDx):
                            if(lg.trovatoSx!="N" and lg.trovatoDx!="B"):
                                pidG.avvio = 0 
