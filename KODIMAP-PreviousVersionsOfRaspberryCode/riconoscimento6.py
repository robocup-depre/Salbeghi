...
class ric:
    def __init__(self,dx, dy , pos, name):
        ...
        self.low_yellow=(18 ,130,95) #10,100,150
        self.up_yellow=(40,255,255)
            ...
            while(self.attivo):
                ...
                if(ret!=True):
                    ...
                    if(self.avvioRiconoscimento == 0):
                        ...
                        minX = int(self.dxframe/2 -40)
                        maxX = int(self.dxframe/2 +40)
                        minY = int(self.dyframe/2 -50)
                        maxY = int(self.dyframe/2 +20)
                        rectc=np.array([[minX,minY],[minX,maxY],[maxX,maxY],[maxX,minY]])
                        cv2.drawContours(self.frame, [rectc], 0, (0,0,255), 1)
                        taglio = frame_gray[minY:maxY,minX:maxX]
                        taglio1 = frame_hsv[minY:maxY,minX:maxX]
                        medianero = np.mean(taglio)
                        #medianero1 = np.mean(np.mean(taglio1, axis = 2))
                        #varnero = np.var(taglio)
                        if(medianero<-1): #60
                            #print(medianero,medianero1)
                            cv2.drawContours(self.frame, [rectc], 0, (0,255,255), 6)
                            trovato = "B"
                        else:
                            ...
                            if(red != 0):
                                ...
                                if(green != 0):
                                    ...
                                    if(yellow != 0):
                                        ...
                                        if(LH != 0):
                                            trovato = 'H'
                                            nam = str(trovato) +" " +self.name + " " +str(random.randint(1000,9999))
                                            mask = np.hstack([mask_black, mask_red, mask_yellow, mask_green])
                                            cv2.imwrite("/home/pi/Desktop/KODIM/FOTO/"+nam +"framein"+".png",framein)
                                            cv2.imwrite("/home/pi/Desktop/KODIM/FOTO/"+nam+"frame"+".png",self.frame)
                                            cv2.imwrite("/home/pi/Desktop/KODIM/FOTO/"+nam+"mask"+".png",mask)
                                        elif(LS != 0):
                                            trovato = 'S'
                                        elif(LU != 0):
                                            trovato = 'U'
