...
class logica():
    ...
    def main(self,q_ric_dx,q_avv_dx,q_ric_sx,q_avv_sx,q_mappa,inRIC,inRIL,tminvit): #funzione principale in multiprocessing
        print("MAIN")
            ...
            if(lg.avvio==0):
                ...
                if(lg.stato == 'primo90' and lg.Libero(0)):
                    ...
                    if(lg.laser[sn.lAvDx] < 15 or lg.laser[sn.lDiDx]<15): #se almeno uno conferma il muro 
                        casella.muri[mappa.limit04(mappa.sempG(robo[g]))] = 1
                    else:
                        casella.muri[mappa.limit04(mappa.sempG(robo[g]))] = 0
                    if(lg.laser[sn.lAvSx] < 15 or lg.laser[sn.lDiSx]<15):
                        casella.muri[mappa.limit04(mappa.sempG(robo[g])+2)] = 1
                    else:
                        casella.muri[mappa.limit04(mappa.sempG(robo[g])+2)] = 0
                    if(lg.laser[sn.lAvCe] < 15):
                        casella.muri[mappa.limit04(mappa.sempG(robo[g])+1)] = 1
                    else:
                        casella.muri[mappa.limit04(mappa.sempG(robo[g])+1)] = 0
                ...
                if(lg.stato == 'zero' and lg.Libero(0)): #se lo stato impostato è 0 e il programma non sta eseguendo nessun'altro programma
                    self.putmappa(maze,0)
                    ...
                    elif(lg.distColCas>=4):
                        casella.colore = lg.casella-1 #lg.nero
                    else:
                    ...
                    if(casella.colore==lg.bianco-1):
                        lg.stato = 'controlloDopoCentra'
                    else:
                        lg.stato = 'inizioScelta'
                        print("CONTROLLO VITTIME EVITATO PER COLORE CASELLA")
                ...
                elif(lg.stato == 'inizioScelta' and lg.Libero(0)):
                    robo[x] = round(robo[x])
                    robo[y] = round(robo[y])
                    lg.avvioLaser = [1]*lg.nLaser
                    lg.stato = 'verificoMuri'
                    lg.statoDopoVM = 'scelta'
                    ...
                    if(casella.muri[mappa.limit04(mappa.sempG(robo[g]) + 3)] == 1):
                        print("ALLINEO")
                    ...
                    pidG.avvio = 1
                    dritto.Avvia(0,1.3,30) # normali 30
                    ...
                    if(finecorsa.stato==0):
                        ...
                        if(finecorsa.stato==1):
                            if(finecorsa.posizione==0): #se è il finecorsa 0 (Dx)
                                gira.Avvia(finecorsa.livello+1, 0.8,-30) #gira di -35 gradi 
                                
                                #gira -35
                            if(finecorsa.posizione==1):
                                gira.Avvia(finecorsa.livello+1,0.8, 30) #se è il finecorsa 1 (Sx)
                                #gira 35
                            finecorsa.stato=2
                        elif(finecorsa.stato==2):
                            dritto.Avvia(finecorsa.livello+1,0.6,10) # vai avati di 16cm
                            finecorsa.stato=3
                        elif(finecorsa.stato==3):
                            if(finecorsa.posizione==0):
                                gira.Avvia(finecorsa.livello+1,0.8,30) #rimettersi dritto, gira di -35 gradi 
                            else:
                                gira.Avvia(finecorsa.livello+1,0.8,-30) #rimettersi dritto, gira di -35 gradi 
                            finecorsa.stato = 4
                        elif(finecorsa.stato == 4):
                            dritto.Avvia(finecorsa.livello+1,-0.6,10) # vai indietro di 12cm
                            finecorsa.stato=5
                ...
                if(allinea.avvio):
                    ...
                    if(allinea.stato==0):
                        ...
                        if(sn.ReadEndRun(sn.fDiDx)==1 and allinea.finecorsaDx==0):  #se è premuto il finecorsa dietro destra e non è ancora stato premuto
                            ...
                            allinea.tempoTotale = 3 #imposto tempo di allinamento ( 2s dal primo finecorsa che si preme)
                ...
                if(controlloVittime.avvio):
                    ...
                    if(controlloVittime.stato==0):
                        ...
                        if((lg.laser[sn.lAvDx] > 15 or lg.laser[sn.lDiDx]>15 ) or
                            lg.avvioCamDx[0]==0 or lg.okCamDx==0 or
                        ...
                        if((lg.laser[sn.lAvSx] > 15 or lg.laser[sn.lDiSx]>15 ) or
                            lg.avvioCamSx[0]==0 or lg.okCamSx==0 or
                        ...
                        if(mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g]))] ==None
                            and sn.Temperatura(sn.tDx)>36
                            and (lg.laser[sn.lAvDx] < 15 and lg.laser[sn.lDiDx] < 15) ):
