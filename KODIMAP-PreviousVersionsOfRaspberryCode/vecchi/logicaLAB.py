...
class logica():
    ...
    def PidThread(self): #ESECUZIONE DID
        ...
        while True:
            if(lg.avvio==0):
                ...
                #print("GYRO:",lg.gradiZ,lg.gradiZ1)
    ...
    def thSensori(self):
        ...
        while(1):
            ...
            if(lg.avvioLaser[nLas]):
                misura = sn.ReadLaser(nLas)
                lg.avvioLaser[nLas] = 0
                if(misura <0):
                    print("ERRORE LASER n:",nLas)
                    lg.avvio = 0
                else:
                    lg.laser[nLas]=misura
            else:
                time.sleep(0.01)
    ...
    def main(self,q_ric_dx,q_avv_dx,q_ric_sx,q_avv_sx,q_mappa): #funzione principale in multiprocessing
        ...
        self.t_sensori = threading.Thread(target=self.thSensori)
        self.t_sensori.daemon = True
        ...
        while(1):
            ...
            if(lg.ricVisuale==0):
                ...
                lg.avvioCamSx[0]=0
            '''
            if(lg.avvioCamDx[0] or lg.avvioCamSx[0] or True):
                sn.AccendiLedCamera()
            else:
                sn.SpegniLedCamera()
            '''
            ...
            if(lg.okCamDx):
                ...
                if(lg.avvioCamDx[0]!=lg.avvioCamDx[1]): #se lo stato è cambito 
                    try:
                        ...
                        if(lg.eccSx>10):
                            print("ECCEZIONE ",lg.avvioCamDx[0]," CAMERA DXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
                        time.sleep(0.1)
                        lg.eccDx += 1
                        if(lg.eccDx>10):
                            lg.okCamDx = 0
            ...
            if(lg.okCamSx):
                if(lg.avvioCamSx[0]!=lg.avvioCamSx[1]):
                    try:
                        ...
                        if(lg.eccSx>1):
                            print("ECCEZIONE ",lg.avvioCamSx[0]," CAMERA SXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
                        time.sleep(0.1)
                        lg.eccSx += 1
                        if(lg.eccSx>10):
                            lg.okCamSx = 0
            ...
            if(lg.avvioGiroscopio or pidG.avvio):
                #lg.gradiZ, lg.gradiY, lg.gradiX = sn.Giroscopio(0)
                lg.avvioGiroscopio = 0
            ...
            if(lg.avvio==0):
                ...
                '''
                if(lg.stato == -1 and lg.Libero(0) and (time.time()-tstartcam)>5):
                    pidG.avvio = 0
                    lg.trovato = "SC"
                    print("SET RILASCIOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
                    if(self.RilevaVittime(0)):
                        lg.stato = -2
                        print("set stato -")
                ...
                ''''
                robo[g] = copy.copy(lg.gradiZ)
                if( lg.velDx == lg.velSx and lg.erroreZ):
                    if(abs(lg.distanza-lg.distanzaold)>15):
                        lg.distanzaold=0
                    lg.difDist = (lg.distanza-lg.distanzaold)*(1.0/30.0)
                ...
                if( lg.velDx == lg.velSx and lg.erroreZ and allinea.fuoriCasella==0):
                    ...
                    #if( (lg.distanza < lg.distanzaold and lg.distanzaold>0) or (lg.distanza > lg.distanzaold and lg.distanzaold<0)):
                ...
                if(vittime.avvio ==0 and (lg.stato == 2.1 or lg.stato == 2.2 or lg.stato == 2.15) and lg.ricVisuale): #or lg.stato == 7
                    lg.avvioCamDx[0]=1
                    lg.avvioCamSx[0]=1
                '''
                if(lg.stato == -1 and lg.Libero(0)):
                    dritto.Avvia(0,-0.4,30)
                    lg.stato = -5
                elif(lg.stato == -5 and lg.Libero(0)):
                    gira.Avvia(0,0.4,-90)
                    lg.stato = 0
                '''
                if(lg.stato == -2 and lg.Libero(0)):
                    pidG.avvio = 0 
                    lg.avvioLaser = [1]*lg.nLaser
                    lg.stato = -1
                elif(lg.stato == -1 and lg.Libero(0)):
                    if(lg.laser[sn.lAvDx] < 15 or lg.laser[sn.lDiDx]<15): #se almeno uno conferma il muro 
                        mappa.map.map[round(robo[x])][round(robo[y])].muri[mappa.limit04(mappa.sempG(robo[g]))] = 1
                    else:
                        mappa.map.map[round(robo[x])][round(robo[y])].muri[mappa.limit04(mappa.sempG(robo[g]))] = 0
                    if(lg.laser[sn.lAvSx] < 15 or lg.laser[sn.lDiSx]<15):
                        mappa.map.map[round(robo[x])][round(robo[y])].muri[mappa.limit04(mappa.sempG(robo[g])+2)] = 1
                    else:
                        mappa.map.map[round(robo[x])][round(robo[y])].muri[mappa.limit04(mappa.sempG(robo[g])+2)] = 0
                    if(lg.laser[sn.lAvCe] < 15):
                        mappa.map.map[round(robo[x])][round(robo[y])].muri[mappa.limit04(mappa.sempG(robo[g])+1)] = 1
                    else:
                        mappa.map.map[round(robo[x])][round(robo[y])].muri[mappa.limit04(mappa.sempG(robo[g])+1)] = 0
                    gira.Avvia(0,1,90)
                    lg.distanza = 0
                    lg.stato = 0
                ...
                if(lg.stato == 0 and lg.Libero(0)): #se lo stato impostato è 0 e il programma non sta eseguendo nessun'altro programma
                    ...
                    mappa.map.map[round(robo[x])][round(robo[y])].colore = lg.casella-1
                    ...
                    lg.stato = 2.1
                    lg.flvittimeprimo = 2.5
                    
                elif(lg.stato == 2.1 and lg.Libero(0)):
                    pidG.avvio = 0 
                    lg.avvioLaser = [1]*lg.nLaser
                    lg.stato = 2.15
                elif(lg.stato == 2.15 and lg.Libero(0)):
                    if((lg.laser[sn.lAvDx] > 15 or lg.laser[sn.lDiDx]>15 ) or lg.avvioCamDx[0]==0 or lg.okCamDx==0 or mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g]))] != None):
                        lg.stato = 2.2
                    elif(lg.fTrovatoDx):
                        if(lg.trovatoDx!="N"):
                            pidG.avvio = 0 
                            self.vittime.Avvia(0,'D'+lg.trovatoDx)
                ...
                elif(lg.stato == 2.2 and lg.Libero(0)):
                    if((lg.laser[sn.lAvSx] > 15 or lg.laser[sn.lDiSx]>15 ) or lg.avvioCamSx[0]==0 or lg.okCamSx==0 or mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g])+2)]!= None):
                        lg.stato = 2.3
                    elif(lg.fTrovatoSx):
                        if(lg.trovatoSx!="N"):
                            pidG.avvio = 0 
                            self.vittime.Avvia(0,'S'+lg.trovatoSx)
                ...
                elif(lg.stato == 2.3 and lg.Libero(0) ):
                    if(mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g]))] ==None  and sn.Temperatura(sn.tDx)>36 and (lg.laser[sn.lAvDx] < 15 and lg.laser[sn.lDiDx] < 15) ):
                        pidG.avvio = 0 
                        self.vittime.Avvia(0,'D'+'C')
                        cprint("TROVATO DX CALORE",'green')
                        mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g]))] = 'C'
                    lg.stato = 2.4
                elif(lg.stato == 2.4 and lg.Libero(0) ):
                    if(mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g])+2)]==None and sn.Temperatura(sn.tSx)>36 and (lg.laser[sn.lAvSx] < 15 and lg.laser[sn.lDiSx] < 15)):
                        pidG.avvio = 0 
                        self.vittime.Avvia(0,'S'+'C')
                        cprint("TROVATO SX CALORE",'green')
                        mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g])+2)] = 'C'
                    lg.stato = lg.flvittimeprimo
                    q_mappa.put([0,maze])
                    '''
                    if(lg.flvittimeprimo==0):
                        lg.stato = 2.5
                    else:
                        lg.stato = 5
                    '''
                        
                elif(lg.stato == 2.5 and lg.Libero(0)):
                    robo[x] = round(robo[x])
                    robo[y] = round(robo[y])
                    lg.stato = 3
                    lg.avvioLaser = [1]*lg.nLaser
                    ...
                    lg.nuoveMisure=0
                    if(lg.laser[sn.lAvDx] < 15 or lg.laser[sn.lDiDx]<15): #se almeno uno conferma il muro 
                        print("MDX",lg.laser[sn.lAvDx],lg.laser[sn.lDiDx])
                        ...
                        mappa.map.map[round(robo[x])][round(robo[y])].muri[mappa.limit04(mappa.sempG(robo[g]))] = 0
                    if(lg.laser[sn.lAvSx] < 15 or lg.laser[sn.lDiSx]<15):
                        print("MSX",lg.laser[sn.lAvSx],lg.laser[sn.lDiSx])
                        lg.pmsx = 1
                        mappa.map.map[round(robo[x])][round(robo[y])].muri[mappa.limit04(mappa.sempG(robo[g])+2)] = 1
                        #lg.avvioCamSx[0]=1
                    else:
                    ...
                    if(lg.noDestra == 0 and lg.pmdx==0): #se posso andare a destra e non c'è il muro
                        ...
                        if(lg.pmsx==0): #se è libero a sinistra
                            ...
                            print("gira 180")
                            if(lg.laser[sn.lAvSx] > lg.laser[sn.lAvDx]):
                    ...
                    lg.stato = 5
                elif(lg.stato == 5 and lg.Libero(0)): #avanti dopo allinea
                    dritto.Avvia(0,0.5,4)
                    
                    lg.stato = 6 #5
                
                elif(lg.stato == 6 and lg.Libero(0)):
                    if(lg.sceltogira==0 or (abs(lg.sceltogira)==180 and lg.okCamSx==1 and lg.okCamDx==1)):
                        lg.stato = 6.5
                    else:
                        lg.flvittimeprimo = 6.5
                        lg.stato = 2.1 #5
                    
                elif(lg.stato == 6.5 and lg.Libero(0)):
                    pidG.avvio = 1
                    dritto.Avvia(0,1.2,30) #altrimenti normali 30
                    lg.stato = 7.5
                elif(lg.stato == 7.5):
                    lg.stato = 7
                    ...
                    lg.avvioLaser[sn.lAvCe]=1
                    if(lg.distanza>15 and lg.azione[0]==dritto.nome):
                        '''
                        if(lg.trovatoDx!='N'):
                            lg.vittimeDx.append(lg.trovatoDx)
                        if(lg.trovatoSx!='N'):
                            lg.vittimeSx.append(lg.trovatoSx)
                        '''
                        if(sn.Temperatura(sn.tDx)>37):
                            lg.vittimeDx.append('C')
                            led.VisualizzaTrovato(1,'C')
                        if(sn.Temperatura(sn.tSx)>37):
                            lg.vittimeDx.append('C')
                            led.VisualizzaTrovato(0,'C')
            
                    
                    if(lg.laser[sn.lAvCe]<=7 and lg.azione[0]!=finecorsa.nome): #se ho un muro vicino davanti e non sto facendo i finecorsa
                    ...
                    elif(lg.inizioRampa != None and (time.time()-lg.inizioRampa) > 2):#c'è una rampa
                        ...
                        if(lg.incRamp==sn.incSalita):
                            mappa.map.map[round(robo[x]-aggX)][round(robo[y]-aggY)].rampa = mappa.limit04(mappa.sempG(robo[g])+1)
                        elif(lg.incRamp==sn.incDiscesa):
                            direz = mappa.limit04(mappa.sempG(robo[g])+1)
                            mappa.map.map[round(robo[x]-aggX)][round(robo[y]-aggY)].rampa = 2-direz+2*(direz%2)
                        dritto.Avvia(0,0.8,10)
                        lg.inizioRampa = None
                    elif(lg.inizioRampa != None and lg.distRamp!=None ):
                        
                        dritto.Avvia(0,0.8,30-lg.distRamp-(time.time()-lg.inizioRampa))
                        lg.inizioRampa = None
                        '''
                        elif(self.RilevaVittime(0) and lg.azione[0]!=rilevavitteme.nome):
                            print("Rilevata una vittima")
                            lg.distVittime = lg.distanza
                        elif(lg.distVittime != None and lg.azione[0]==rilevavitteme.nome):
                            lg.avvioCamDx[0]=0
                            lg.avvioCamSx[0]=0
                            if(lg.Libero(0)):
                                dritto.Avvia(0,0.8,30-lg.distVittime)
                                lg.distVittime = None
                        '''
                    elif( sn.ReadEndRun(sn.fAvDx) and lg.azione[0]!=finecorsa.nome and g.laser[sn.lAvCe]>7.5): #premuto finecorsa avanti di Dx
                        print("finecorsasx")
                        pidG.avvio = 1
                        finecorsa.AvviaAvDx(0)
                    elif( sn.ReadEndRun(sn.fAvSx) and lg.azione[0]!=finecorsa.nome and g.laser[sn.lAvCe]>7.5):
                        print("finecorsaSx")
                        pidG.avvio = 1
                        ...
                        lg.stato = 7.8
                        print("esci per stato 7.5",lg.laser[sn.lAvCe])

                elif(lg.stato ==7.8 and lg.Libero(0)):
                    
                    lg.ColoreCasella() #eseguo il calcolo per trovare il colore della casella (risultato in lg.casella)
                    if(lg.casella==lg.bianco and False):
                        print("ok controllo vittime")
                        lg.stato = 7.9
                        lg.avvioLaser = [1]*lg.nLaser
                        if(lg.trovatoDx!='N'):
                            lg.vittimeDx.append(lg.trovatoDx)
                        ...
                        lg.stato = 8
                elif(lg.stato ==7.9 and lg.Libero(0)):
                    lg.stato = 7.95
                    if(lg.laser[sn.lAvDx] < 15 and lg.laser[sn.lDiDx]<15):
                        
                        diz={}
                        for tipo in vittime.tipi:
                            #print("controllo telec Dx",tipo,lg.vittimeDx.count(tipo),diz)
                            diz[tipo]= lg.vittimeDx.count(tipo)
                        trovdx = max(diz,key=diz.get)
                        voltdx = diz[trovdx]
                        print("RILEVAZIONE VITTIME DX:",lg.vittimeDx,diz)
                        print("TROVATE:",trovdx,"volt:",voltdx)
                    else:
                        
                        trovdx = None
                        voltdx = 0
                        
                    if(voltdx>2 or trovdx=='H' or trovdx=='S' or trovdx=='U'):
                        self.vittime.Avvia(0,'D'+trovdx)
                        cprint("SCELTO DX :" + str(trovdx),'green')
                        mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g]))] = trovdx
                        
                elif(lg.stato ==7.95 and lg.Libero(0)):
                    lg.stato = 8
                    if(lg.laser[sn.lAvSx] < 15 and lg.laser[sn.lDiSx]<15):
                        diz2={}
                        for tipo in vittime.tipi:
                            diz2[tipo]= lg.vittimeSx.count(tipo)
                        trovsx = max(diz2,key=diz2.get)
                        voltsx = diz2[trovsx]
                        print("RILEVAZIONE VITTIME SX:",lg.vittimeSx,diz2)
                        print("TROVATE:",trovsx,"volt:",voltsx)
                    else:
                        trovsx = None
                        voltsx = 0
                    ...
                    if(voltsx>2 or trovsx=='H' or trovsx=='S' or trovsx=='U'):
                        self.vittime.Avvia(0,'S'+trovsx)
                        cprint("SCELTO SX :" +str(trovsx),'green')
                        mappa.map.map[round(robo[x])][round(robo[y])].vittime[mappa.limit04(mappa.sempG(robo[g])+2)] = trovsx
                ...
                if(vittime.avvio):
                    ...
                    if(vittime.stato==0):
                        ...
                        rk.MotRuota(-100,200)#orno didietro di 100 rotazioni (0.5mm)
                        ...
                        time.sleep(0.6)
                        vittime.rotaz = rk.MotAvvio(100) #porto avanti il cursore
                        ...
                        print(vittime.nome," bloccato st:",vittime.stato)
                        rk.MotStop()
                        rk.stopServo()
                        vittime.avvio = 0
